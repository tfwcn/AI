...
def main():
    ...
    # 卷积实现图片分类
    model = tf.keras.models.Sequential([
        tf.keras.layers.Conv2D(32, (3, 3), padding='same', use_bias=False, input_shape=(28, 28, 1)),
        tf.keras.layers.BatchNormalization(),
        tf.keras.layers.Activation(activation=tf.keras.activations.relu),
        # tf.keras.layers.ZeroPadding2D(((1,0),(1,0))),
        # tf.keras.layers.Conv2D(128, (3, 3), strides=(2, 2), padding='valid', use_bias=False),
        # tf.keras.layers.BatchNormalization(),
        # tf.keras.layers.Activation(activation=tf.keras.activations.relu),
        ...
        tf.keras.layers.Activation(activation=tf.keras.activations.relu),
        # tf.keras.layers.ZeroPadding2D(((1,0),(1,0))),
        # tf.keras.layers.Conv2D(10, (3, 3), strides=(2, 2), padding='valid', use_bias=False),
        # tf.keras.layers.BatchNormalization(),
        # tf.keras.layers.Activation(activation=tf.keras.activations.relu),
        # tf.keras.layers.MaxPooling2D((2,2)),
        tf.keras.layers.AveragePooling2D((2,2)),
